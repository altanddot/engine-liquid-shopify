  ...
  usePatternLabConfig: function(config) {
    ...
    engine.registerTag('section', {
      parse: function (token) {
        ...
        this.sectionData = {};
        const sectionDataFilePath = path.join(process.cwd(), patternsPath, 'sections', `${this.template}.json`);

        if (fs.existsSync(sectionDataFilePath)) {
          this.sectionData = JSON.parse(fs.readFileSync(sectionDataFilePath));

          if (this.sectionData && this.sectionData.section) {
            this.sectionData.section = {
              ...this.sectionData.section,
              // id: '123', // @TODO: generate random id
              blocks: this.sectionData.section.blocks.map(block => ({
                ...block,
